...
class AudioTranscriptionModel:
    ...
    def transcribe_batch(self, audio_paths: list[str]) -> list[str]:
        return [{'file_path': str(audio_path), 'result': self.transcribe(audio_path)} for audio_path in audio_paths]
    
    def _write_res_to_dir(self, res: list[str], out_dir: str) -> None:
        out_dir = Path(out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        for r in res:
            with open(out_dir / (r['file_path'].split('/')[-1].split('.')[0] + '.txt'), 'w') as f:
                f.write(r['result'])
    
    def transcribe_files_in_directory(self, input_dir: str, out_dir: str=None) -> list[str]:
